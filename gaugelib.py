...
class DrawGauge2(ini):
    def __init__(self, parent,
        ...
        self.canvas = tk.Canvas(self, width=self.size, height=self.size - self.size / 12, bg=bg_col,
    ...
    def draw_tick(self, divisions=100):
        ...
        label = self.unit
        self.canvas.create_text(self.size / 2, 2 * self.size / 5, font=("Arial", int(self.size / 20)), fill="white",
                                text=label, angle=0)
        label = 'SaraKhoram'
        self.canvas.create_text(self.size / 2, 3 * self.size / 5, font=("Arial", int(self.size / 18), 'bold'),
                                fill="red", text=label, angle=0)
        self.readout = self.canvas.create_text(self.size / 2, 4 * self.size / 5,
                                               font=("Arial", int(self.size / 18), 'bold'), fill="white", text='')
        ...
        for tick in range(divisions + 1):
            ...
            if (tick % 10) == 0:
                ...
                if int(label) != 112 | int(label) != 125:
    ...
    def set_value(self, number: (float, int)):
        ...
        self.readout = self.canvas.create_text(self.size / 2, 4 * self.size / 5,
                                               font=("Arial", int(self.size / 14), 'bold'), fill="white", text=label,
                                               angle=0)
...
class DrawGauge3(ini):
    ...
    def draw_tick(self, divisions=100):
        ...
        outer_tick_radius = int((self.size - self.size / 9) * 0.45)
        label = self.unit
        self.canvas.create_text(self.size / 2, 3 * self.size / 10, font=("Arial", int(self.size / 20)), fill="red",
                                text=label, angle=0)
        label = 'Ardiotech'
        self.canvas.create_text(self.size / 2, 4 * self.size / 10, font=("Arial", int(self.size / 18), 'bold'),
                                fill="light blue", text=label, angle=0)
        self.readout = self.canvas.create_text(self.size / 2, 4 * self.size / 5,
                                               font=("Arial", int(self.size / 18), 'bold'), fill="blue", text='')
        inner_tick_radius2 = int((self.size - self.size / 9) * 0.48)
        outer_tick_radius2 = int((self.size - self.size / 9) * 0.50)
        inner_tick_radius3 = int((self.size - self.size / 9) * 0.35)
        outer_tick_radius3 = int((self.size - self.size / 9) * 0.40)
    ...
    def set_value(self, number: (float, int)):
        ...
        label = str('%.2f' % number)
        self.canvas.delete(self.readout)
        self.readout = self.canvas.create_text(self.size / 3 - 5, self.size / 2 + 5,
                                               font=("Arial", int(self.size / 16), 'bold'), fill="white", text=label,
                                               angle=0)
